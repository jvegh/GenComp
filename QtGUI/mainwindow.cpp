...
//  GenCompSettings::setValue(GENCOMP_SETTING_HAS_SAVEFILE, false);
//  GenCompSettings::setValue(GENCOMP_SETTING_SAVEPATH, "");
}
...
/*  auto *proc = ProcessorHandler::get();
  ...
  if (static_cast<EditTab *>(m_tabWidgets.at(EditTabID).tab)
    ...
    saveMsgBox.setWindowTitle("GenComp");
  ...
  // close
  GenCompSettings::setValue(GENCOMP_GLOBALSIGNAL_QUIT, 0);
...
/*  static_cast<ProcessorTab *>(m_tabWidgets.at(ProcessorTabID).tab)->pause();
  ...
  if (static_cast<EditTab *>(m_tabWidgets.at(EditTabID).tab)
    ...
    GenCompSettings::setValue(GENCOMP_SETTING_SAVEPATH, savePath);
    GenCompSettings::setValue(GENCOMP_SETTING_HAS_SAVEFILE, true);
  }
  */
}
...
/*  SaveDialog diag(
  ...
  if (!GenCompSettings::value(GENCOMP_SETTING_HAS_SAVEFILE).toBool()) {
...
/*  QMessageBox mbox;
  ...
  if (!static_cast<EditTab *>(m_tabWidgets.at(EditTabID).tab)
           ->getAssemblyText()
           .isEmpty() ||
      GenCompSettings::value(GENCOMP_SETTING_HAS_SAVEFILE).toBool()) {
