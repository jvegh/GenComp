...
#define LOG_TRACE_SIGNAL(TX,VALUE)
    ...
    #define LOG_TRACE(TX) \
        if(!UNIT_TESTING){std::cerr << PrologString_Get().c_str() << "\"" << std::string(TX).c_str() << "\"" <<endl;}
    #define LOG_TRACE_SOURCE(TX) \
        if(!UNIT_TESTING){std::cerr << PrologString_Get().c_str() << "\"" << std::string(TX).c_str() << "\"" "@line " << LOG_LINENO(PC_Get()) << ":\n" \
        << "      " << LOG_SOURCELINE(PC_Get()) << "\n";}
    #define LOG_TRACE_FETCH_SOURCE(TX) \
        if(!UNIT_TESTING){std::cerr << Prologstring_Get().c_str() << "\"" << std::string(TX).c_str() << "\"" "@line " << LOG_LINENO(fetchPC_Get()) << ":\n" \
    ...
    #define LOG_HEXA(TX,VALUE) \
        if(!UNIT_TESTING){std::cerr << PrologString_Get().c_str() << " !! " << std::string(TX).c_str() << hex << "0x" << VALUE <<  dec << "(" << VALUE << ")\"\n";}
    #define LOG_TRACE_HEXA(TX,VALUE) \
        if(!UNIT_TESTING){std::cerr << PrologString_Get().c_str() << " !! " << std::string(TX).c_str() << hex << "0x" << VALUE << dec << "\" @line " << LOG_LINENO(PC_Get()) << ":\n" \
                    << "     " << LOG_SOURCELINE(PC_Get()) << "\n";}
...
#define DEBUG_LOCATION " //<" <<  string(__FILE__).substr(string(__FILE__).find_last_of("/") + 1) << ':'  << dec << __LINE__  
    ...
    #define DEBUG_EVENT_OBJECT(x)    if(!UNIT_TESTING) \
        std::cerr  << "EVT" << PrologString_Get().c_str() << " " << x  << DEBUG_LOCATION << std::endl
  #define DEBUG_FETCH_EVENT(x)    if(!UNIT_TESTING) \
        std::cerr  << "EVT" << PrologString_Get().c_str() << " " << x << " //" << " <" << string(__FILE__).substr(string(__FILE__).find_last_of("/") + 1) << ':'  << dec << __LINE__  << std::endl
